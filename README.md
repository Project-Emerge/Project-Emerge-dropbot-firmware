...
They share the UWB stack through three library crates, so a change to the radio protocol is a
...
on its own set of topics without colliding with the others. The exceptions are
`/config/tag-assignments` and `/config/anchors`, which are shared by the whole fleet on purpose
(see below).
...
|---|---|---|---|
| `/pose/{id}` | ~18 Hz (once per UWB superframe) | `x_m`, `y_m`, `heading_rad`, `speed_m_s`, `position_variance_m2`, `anchors_used`, timestamp | **The robot's own fused position estimate**, from the onboard trilateration + unicycle-EKF filter in [`crates/dropbot-estimation`](crates/dropbot-estimation). Computed onboard rather than on a server: that is where it is needed for closed-loop control, a Wi-Fi round trip is slower than the ranging period it would be correcting, and a broker outage should not cost the robot its position. Read `position_variance_m2` as "how much should this be trusted" and `anchors_used` as coverage -- two or fewer means the fix is leaning on the motion model. |
...
| `/uwb/{id}` | **Off by default**; up to 4 messages per ~54 ms superframe when enabled | Anchor ID, distance in millimetres, sequence, timestamp, applied clock-ratio correction and whether it had converged | One message per accepted range. This is the raw input the pose is computed from, and the data the per-anchor range calibration (`uwb_protocol::RangeBias`) is fitted from. Gated behind `PUBLISH_RAW_RANGES` in [src/tasks/pose_estimator.rs](src/tasks/pose_estimator.rs): turn it on for a calibration session, off in a running swarm. |
...
| `/config/anchors` | `robot_antenna_height_m` plus a list of `{ anchor_id, x, y, z, offset_mm, scale_ppm }` | Retained message carrying the arena's surveyed anchor geometry and per-anchor range calibration. **Required for a pose**: unlike the tag assignments, the compiled default here is deliberately *empty*, so a robot with no configuration publishes no pose rather than a confident pose in the wrong coordinate frame. Fleet-shared because the anchors are physically shared hardware -- two robots holding different geometries would report positions in two different frames. |
