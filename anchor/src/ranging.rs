...
/// drown the RTT channel -- roughly 5.5 s at the 54.4 ms superframe.
...
pub async fn run(pins: pins::UwbPins, anchor_index: usize) -> ! {
    let anchor_id = uwb_protocol::ANCHOR_IDS[anchor_index];
    let is_master = anchor_id == uwb_protocol::MASTER_ANCHOR_ID;
    // This board's own calibrated pair -- see `drivers::uwb::ANTENNA_DELAYS`. Read once here and
    // threaded through, rather than looked up at each use, so there is one place that decides which
    // anchor's calibration this build applies.
    let antenna_delay = uwb::antenna_delay(anchor_index);
