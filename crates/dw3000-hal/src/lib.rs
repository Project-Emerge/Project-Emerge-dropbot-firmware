...
/// size of 8.
///
/// # Why 128 preamble symbols and not the 64 this started at
///
/// The preamble is what the CIA accumulates the channel impulse response from, so its length sets
/// how well the leading edge -- the first path, the only part of the response that carries the
/// distance -- stands out of the noise. 64 symbols is the *shortest* the part offers, meant for
/// high-rate data links rather than ranging, and it leaves the first-path timestamp visibly noisy:
/// on a stationary robot the four anchors measured 16 to 41 mm of range sigma at 64 symbols.
///
/// The cost is air time, and it is small where it matters. 128 symbols is ~65 us more per frame,
/// which widens [`uwb_protocol::MAX_FRAME_AIRTIME_US`] but not `SUBSLOT_US`: the sub-slot has to
/// cover a frame's air time plus the robot's readout, and that readout measured 100-330 us against
/// a 1000 us sub-slot, leaving over 500 us of margin at each boundary. So the whole change costs
/// about 1 ms of superframe, i.e. 14.3 Hz to 14.1 Hz.
///
/// It also fixes something for free. `Config::default()` sets `sfd_timeout: 129`, and the crate's
/// own formula for it is `preamble + 1 + sfd_len - pac`: with a 64-symbol preamble that is 65, so
/// the receiver was holding roughly twice as long as it should after a false preamble detection
/// before giving up and rearming. At 128 symbols the default *is* the correct value.
...
/// The same receive lifecycle as [`Received`], decoded by a protocol-specific callback.
///
/// The production and calibration fixture protocols intentionally have incompatible headers. This
/// generic entry point lets the temporary fixture reuse all IRQ/error cleanup without teaching the
/// production [`receive_packet`] path to accept calibration frames.
pub struct DecodedReceived<SPI, T> {
    pub radio: Radio<SPI>,
    pub outcome: Result<(T, u64), RxMiss>,
    pub stale: Option<&'static str>,
}
...
/// Receives one frame and hands its payload to `decode`.
///
/// A decoder returning `None` is reported as [`RxMiss::Undecodable`]. Callers should pass exactly
/// one protocol decoder; in particular, the fixture passes `CalibrationPacket::decode`, never a
/// production-or-fixture fallback.
pub async fn receive_decoded<SPI, IRQ, T>(
    mut radio: Radio<SPI>,
    irq: &mut IRQ,
    config: Config,
    timeout: Duration,
    decode: impl Fn(&[u8]) -> Option<T>,
) -> Result<DecodedReceived<SPI, T>, Error<SPI>>
where
    SPI: SpiDevice<u8>,
    IRQ: Wait,
{
    ...
    let decoded = match receiving.r_wait(&mut buffer).await {
        Ok(message) => match message.frame.payload() {
            Some(payload) => decode(payload)
                .map(|packet| (packet, message.rx_time.value()))
                .ok_or(RxMiss::Undecodable {
                    payload_len: payload.len(),
                }),
            None => Err(RxMiss::Undecodable { payload_len: 0 }),
        },
