...
//! roughly **18 Hz**. It also moves every tight deadline onto the anchors: the robot's only
...
///
/// Channel 5, 64 MHz PRF, 64-symbol preamble, 6.8 Mbps: 64 preamble symbols at 1017.63 ns
/// (65.1 us), an 8-symbol IEEE short SFD (8.1 us), a 21.0 us PHR, and the data phase. The longest
...
/// which at 6.8 Mbps with Reed-Solomon parity is roughly 50 us. That totals ~145 us; 200 us leaves
/// room for the inter-frame spacing the radio inserts and for a longer payload later.
pub const MAX_FRAME_AIRTIME_US: u32 = 200;
...
///
/// Up from 700 us: `tasks::uwb_ranging`'s `LOG_RECEPTION_TIMING` measured the robot's own
/// arm-decode-rearm cycle in `run_slot`'s reception loop -- reading a frame out over SPI, not just
/// its air time -- running close to or past 700 us on this RV32IMAC core (soft-float, no hardware
/// wide multiply, and the ranging maths is 128-bit fixed-point). At 700 us that cost eats into the
/// *next* sub-slot's window before the receiver is rearmed, capping every superframe at two
/// responses regardless of how many anchors actually answered -- confirmed by anchor-side logs
/// showing all four anchors replying cleanly every time they hear a `Poll`. 1000 us leaves real
/// margin over the measured worst case instead of the bare minimum.
...
pub const fn response_offset(anchor_index: usize, sequence: u16) -> u32 {
    let subslot = (anchor_index + sequence as usize) % ACTIVE_ANCHOR_COUNT;
...
mod tests {
    ...
    fn superframe_matches_the_documented_rate() {
        assert_eq!(ROBOT_SLOT_US, 200 + 800 + 4 * 1_000 + 400);
        assert_eq!(SUPERFRAME_US, 3_000 + 12 * ROBOT_SLOT_US + 2_000);
        assert_eq!(SUPERFRAME_US, 69_800);
        // ~14.3 Hz, against the 3.65 Hz of the anchor-initiated revision this replaced.
