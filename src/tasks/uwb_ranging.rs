...
//! doing nothing else. The rate goes to ~18 Hz and, because a robot slot no longer contains one
...
/// drift across 54.4 ms is barely 1 us, so this is almost entirely about MCU timer jitter and this
...
/// How often to log a ranging summary: roughly 5.5 s at the 54.4 ms superframe.
const REPORT_EVERY_SUPERFRAMES: u32 = 100;

/// Whether to log every accepted range's raw distance as it's computed.
///
/// Off by default: at up to four anchors per superframe and ~18 Hz this is a lot of log lines. On
/// while diagnosing why no superframe has produced enough simultaneous anchors to bootstrap a pose --
/// see `tasks::pose_estimator`'s `PUBLISH_RAW_RANGES`, which this mirrors for the serial console
/// instead of MQTT. Turn back off once that's resolved.
///
/// Off again now that it has served its purpose: every logged superframe showed exactly two of the
/// four anchors, never more, never fewer -- too regular to be independent RF loss. `LOG_RECEPTION_TIMING`
/// below is the next measurement, and this call sits inside the loop it's timing, so leaving it on
/// would add its own logging cost to exactly the thing being measured.
const LOG_RAW_RANGES: bool = false;
...
///
/// Anchors are spaced `SUBSLOT_US` (700 us) apart on air, confirmed clean by every anchor's own
/// stats (0 response-tx failures, ~350 us of headroom against their 800 us budget). But the robot
/// only ever captures exactly two of the four per superframe -- never one, three, or four -- which is
/// too regular to be independent per-anchor RF loss and points instead at the robot's own receive
/// loop falling behind: each `uwb::receive_packet` call does a real SPI round trip to read out and
/// decode a frame before the next one can arm, and if that costs more than one sub-slot, every
/// successful catch pushes the next attempt's arm-time past the following sub-slot's window. This
/// logs exactly that: where in time (relative to the `Poll`) each of the four attempts actually armed,
/// against the sub-slots' known boundaries (1000, 1700, 2400, 3100 us after `poll_sent_at`, from
/// `MAX_FRAME_AIRTIME_US + RESPONSE_GUARD_US + n * SUBSLOT_US`).
...
async fn range_uwb(
    ...
    loop {
        ...
        let mut radio = match uwb::bring_up(spi_device, tag_id).await {
            Ok(radio) => radio,
            Err(e) => {
        ...
        'session: loop {
            ...
            // of at most twelve entries, against a ~54 ms cycle.
...
async fn run_slot(
    ...
    let len = Packet::Poll {
        ...
        poll_tx: uwb_protocol::delayed_tx_timestamp(poll_tx_ticks, uwb::ANTENNA_DELAY.tx),
    }
    .encode(&mut buffer);
    ...
    //
    // Deliberately minimal work inside this loop: `LOG_RECEPTION_TIMING` measured every successful
    // catch pushing the next attempt's arm-time past the *following* sub-slot's window, capping this
    // robot at two of four responses regardless of how many anchors actually answered. The clock-rate
    // update, the ranging maths (128-bit fixed-point) and the channel send don't need to happen before
    // the next sub-slot is armed -- only identifying which anchor answered and stashing its raw
    // timestamps does -- so everything else moves below, after all four attempts are done.
    let mut radio = radio;
    ...
    for reception in 0..uwb_protocol::ACTIVE_ANCHOR_COUNT {
        // The first `Response` is up to a whole `RESPONSE_GUARD_US` away, since that is the anchor
        // turnaround; each subsequent one is one sub-slot behind the last.
        let timeout_us = if reception == 0 {
            uwb_protocol::RESPONSE_GUARD_US + uwb_protocol::MAX_FRAME_AIRTIME_US
        } else {
            uwb_protocol::SUBSLOT_RX_TIMEOUT_US
        };
        reception_armed_at_us[reception] =
            Instant::now().duration_since(poll_sent_at).as_micros() as u32;
        let (next_radio, received) =
            uwb::receive_packet(radio, irq, config, Duration::from_micros(timeout_us.into()))
                .await?;
        radio = next_radio;
        reception_hit[reception] = received.is_some();
    ...
    for (anchor_index, ticks) in response_ticks.iter().enumerate() {
        ...
        match uwb_protocol::distance_mm_ss_twr(poll_tx, poll_rx, response_tx, response_rx, ratio) {
            Some(distance_mm) => {
                ...
                if LOG_RAW_RANGES {
                    info!(
                        "uwb: raw range from anchor {:04x}: {} mm, sequence {}, converged {}",
                        anchor_id, distance_mm, sequence, converged
                    );
                }
