...
pub struct RangeMeasurement {
    ...
    pub distance_mm: u32,
    pub sequence: u16,
    /// Rotating anchor response sub-slot, `0..ACTIVE_ANCHOR_COUNT`.
    ///
    /// With single-sided TWR a residual clock-ratio error scales with reply delay, hence with this
    /// value. A range error that follows `response_subslot` is a clock-correction problem; one that
    /// follows robot heading instead is an RF/polarization problem. Publishing it makes the two
    /// distinguishable in an MQTT calibration capture.
