...
/// The dropbot's own antenna delay, in DW3000 clock ticks.
///
/// Currently [`AntennaDelay::NOMINAL`], which is a starting point and not a calibration -- see
/// that constant's docs for the tick-to-millimetre scaling. It is called out separately here
/// because the dropbot's DWM3000 sits in a different RF environment from the anchors' (next to two
/// motors, a battery and a Wi-Fi antenna) and will not end up sharing their calibrated value.
pub const ANTENNA_DELAY: AntennaDelay = AntennaDelay::NOMINAL;
...
pub async fn bring_up(spi_device: UwbSpiDevice, tag_id: u16) -> Result<Radio, Error<UwbSpiDevice>> {
    dw3000_hal::bring_up(
        spi_device,
        tag_id,
        ANTENNA_DELAY,
